...
export async function loadAutoReplyConfig(): Promise<ConfigMap> {
...
/**
 * Core auto-reply engine: sync reviews, generate AI replies, post to Google.
 * Used by both the Vercel cron handler and the manual admin trigger.
 */
export async function runAutoReply(config: ConfigMap): Promise<AutoReplyResult> {
  ...
  if (tokenRows.length === 0) {
    return { success: false, synced: 0, generated: 0, posted: 0, message: 'Google not connected' };
  }

  const token = tokenRows[0];
  if (!token.account_name || !token.location_name) {
    return { success: false, synced: 0, generated: 0, posted: 0, message: 'No account/location in token data' };
  }
...
`;
  ...
  const oldLastRun = config.autoreply_last_cron_run || null;
  await sql`
    INSERT INTO system_config (key, value, updated_at)
    VALUES ('autoreply_last_cron_run', ${now}, NOW())
    ON CONFLICT (key) DO UPDATE SET value = ${now}, updated_at = NOW()
  `;

  // ── Audit log entry ─────────────────────────────────────────────────
  const summary = `OK: ${synced} synced, ${generated} generated, ${posted} posted`;
  await sql`
    INSERT INTO config_audit_log (config_key, old_value, new_value)
    VALUES ('autoreply_last_cron_run', ${oldLastRun}, ${summary})
  `;

  return { success: true, synced, generated, posted, message: summary };
}
